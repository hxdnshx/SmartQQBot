...
+ 立即获取深秘录Lu难度揭示板&Twitter可连接的ip(输入```我要围观```或```SnprBot```触发)

+ 记录群内发出过的ip(包括深秘录(Snpr),非想天则(FXTZ))

+ 将现在群内发出过的可以连接的ip显示出来(输入"来个人打"触发)

+ 检验一个ip的当前状态(输入```check {192.168.1.1:10800}```触发,可以说明这个ip是否可以用录/则连接,录可额外做到检测是否能围观)

+ 检验一个用户的tenco录的战绩数据(输入```tenco {用户名}```触发)

+ 检验一个用户的tenco则的战绩数据(输入```tanco {用户名}```触发)
